...
class ContextAggregationModule(nn.Module):
    def __init__(self, channels):
        ...
        self.conv1 = nn.Conv1d(channels, channels, 1, dilation=1)
        self.conv2 = nn.Conv1d(channels, channels, 1, dilation=2)
        self.conv3 = nn.Conv1d(channels, channels, 1, dilation=4)
    ...
    def forward(self, x):
        ...
        out2 = self.conv2(x)
        out3 = self.conv3(x)
        out = out1 + out2 + out3  # Aggregate multi-scale context
        out = self.bn(out)
        return F.relu(out)
